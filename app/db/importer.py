...
def import_to_neo4j(conn, cities, distances):
    ...
    try:
        ...
        for city in cities:
            ...
            RETURN c
            """
            conn.run_query(query, {"name": city})
...
def add_city_coordinates(conn, city_coordinates):
    ...
    try:
        for city, coords in city_coordinates.items():
            ...
            RETURN c
            """
            conn.run_query(query, {"name": city, "lat": coords[0], "lon": coords[1]})

        logger.info(f"Se actualizaron {len(city_coordinates)} ciudades con coordenadas")
        return True
    except Exception as e:
...
def main():
    ...
    csv_file = os.path.join(BASE_DIR, "data", "distancias_ecuador.csv")

    # Procesar el archivo CSV
    cities, distances = process_csv_file(csv_file)
    ...
    if import_to_neo4j(conn, cities, distances):
        ...
        if add_city_coordinates(conn, city_coords):
